...
class HTML2PDF_locale
{
    ...
    static public function load($code)
    {
        ...
        // the file must exist
        if (!is_file($file)) {
            throw new HTML2PDF_exception(0, 'language code ['.self::$_code.'] unknown. You can create the translation file ['.$file.']');
        }
        ...
        while (!feof($handle)) {
            $line = fgetcsv($handle);
            //if (count($line)!=2) continue; "ESTA LINEA GENERABA ERROR - NO TRADUCIA LA PAGINA A PDF"
